...
get_posterior<-function(nameA='spA', nameB='spB', nSubdir=10, sub_dir_sim='iteration_2', model='estimation_1', sub_dir_model=1, nPosterior=1000, figure=FALSE, timeStamp='SI_2N', path2observation='SI_2N/iteration_2/pod_0', do_nnet=T, useSFS=0){
	...
	obs_ss_tmp = read.table(paste(path2observation, '/obs_stat.txt', sep=''), h=T)
	...
	if(useSFS == 1){
		sfs_obs = read.table(paste(path2observation, '/obs_sfs.txt', sep=''), h=T)
	...
	tmp = tmp[, -which(colnames(tmp)=='Na')] # remove the Na paramater because fixed to 1
	nParams = ncol(tmp)
	...
	for(rep in seq(0, nSubdir-1, 1)){
		...
		tmp_params = tmp_params[, -which(colnames(tmp_params)=='Na')] # remove the Na paramater because fixed to 1
