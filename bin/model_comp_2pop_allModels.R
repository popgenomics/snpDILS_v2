...
obs_ss = read.table(paste(timeStamp, '/obs_stat.txt', sep=''), h=T)[, ss] # global statistics over all loci (avg and std)
obs_sfs = read.table(paste(timeStamp, '/obs_sfs.txt', sep=''), h=T)
...
for(m in models){
	...
	for(rep in seq(0, nSubdir-1, 1)){
		...
		if(file.exists(paste(timeStamp, '/', sub_dir_sim, '/', m, '_', rep, '/priorfile.txt', sep='')) == TRUE){
			...
			tmp_params = tmp_params[, -which(colnames(tmp_params)=='Na')]
